...
<p align="center">
  <b>AI 驱动的 X 旁路扩展 · 让 X 再次值得刷。</b><br>
  <code>AI-Native</code> · <code>Ambient</code> · <code>Zero-PII</code> · <code>Open Source AGPL-3.0</code>
</p>
...
2026 年了，刷 X 还是像在垃圾堆里翻硬币：

- 评论区被广告号、色情引流号淹没——正常讨论沉底
- 想关注一个新 KOL，分不清真号 / 营销号 / 水军号
- 算法决定你看到谁，而不是你决定
- 想找一个人聊过什么、最热几条是什么——只能手动翻几十层

**Make X Great Again (MXGA)** 是一个 **AI 驱动的 X 旁路扩展**。
它的设计哲学是 **Ambient AI**：你只管正常刷 X，AI 在背景静默工作，**不主动提示、不打断、不要求你输入任何东西**。
凡是会动你账号的动作（拉黑、屏蔽），都得**你**亲自按一下；扩展只把决策成本压到最低。

> **Zero-PII**（不收集任何个人信息）· **Privacy-first**（GH 登录只为防滥用计数）· **Open Source**（每行代码可查，AGPL-3.0）
